...
export async function createNote(title: string, text: string, expiresAt: Date | null = null): Promise<string> {
      ...
      const inserted = await pool.insert(notes).values({
        title,
        text,
        expiresAt,
        url,
...
export async function getNoteById(id: string) {
  const result = await pool.select().from(notes).where(eq(notes.id, parseInt(id)));
  return result[0];
}

export async function getNoteByUrl(url: string) {
  const result = await pool.select().from(notes).where(eq(notes.url, url));
  return result[0];
}

export async function removeExpiredNotes() {
  const result = await pool.delete(notes).where(
